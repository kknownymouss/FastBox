...
import "../../static/css/FolderIconComponent.css"

function FolderIconComponent(props) {
    ...
    // input states
    const [newName, setNewName] = useState(props.idMapping[props.folderId].name)
    ...
    const handleOpenMenu = (event) => {
        ...
        setOpenMenu(true)
        event.stopPropagation() // stops propagation so that the click doesnt result in moving to a new folder
    }

    // updates move menu state
    ...
    function deleteFolder(event) {
        fetch(`${API_URL}/delete_file`, {
            ...
            body: JSON.stringify({
                folder_file_id: props.folderId,
                box_code: props.boxCode,
                absolute_box_id: props.idMapping[props.folderId].absolute_box,
    ...
    function fetchMoveFolder(event) {
    ...
    return(
        <div onClick={() => {if (!openMenu && !openRename && !openMove && !openInfo) {props.updateActiveFolder(props.folderId)}}} className={props.gridView ? "folder-icon-wrap" : "folder-icon-wrap-inline"}>
            <img className={props.gridView ? "red-folder-svg" : "red-folder-svg-inline"} src={redFolder} />
            <p style={{fontSize: "14px" , textOverflow: "ellipsis", whiteSpace: "nowrap", overflow: "hidden", width: "100%"}} className="poppins bold">{props.idMapping[props.folderId].name}</p>
            <span onClick={handleOpenMenu} style={{position: "absolute", top: "5px", right: "5px", userSelect: "none"}} className="material-icons">more_horiz</span>
            {openMenu ? <div className="folder-icon-menu swing-in-top-fwd" style={{right: props.gridView ? "" : "10px", top: props.gridView ? "" : "0px"}}>
                ...
                {props.fullActivityState && <div onClick={openMoveFolder} className="folder-icon-menu-option" style={{display: "flex", alignItems: "center"}}>
                    ...
                    <p className="poppins bold">Move Folder</p>
                </div>}
                ...
                {props.fullActivityState && <div onClick={deleteFolder}  className="folder-icon-menu-option" style={{display: "flex", alignItems: "center"}}>
                    <span style={{color: "rgb(54, 53, 53)", marginRight: "10px"}} class="material-icons">
                        delete
                    </span>
            ...
            {openRename && <div className="rename-popup-div swing-in-top-fwd">
            ...
            {openMove ? 
                <div style={{textAlign: "center"}} onClick={event => event.stopPropagation()} className="move-popup-div swing-in-top-fwd">
                    <p className="lato" style={{fontSize: "20px", marginBottom: "5px"}}>" {props.idMapping[props.folderId].name} "</p>
                    ...
                    <div className="new-folder-popup-buttons">
                        ...
                        <button onClick={fetchMoveFolder} style={{backgroundColor: "var(--light-red)", color: "white", border: "none"}} className="new-folder-popup-button poppins bold">Move To {props.idMapping[activeMoveFolder].name}</button>
                    </div>
                </div>
                : null}
                {openInfo &&
                    <div onClick={event => event.stopPropagation()} className="info-popup-div swing-in-top-fwd">
                        <div style={{display: "flex", marginBottom: "2em"}}>
                            <div style={{paddingRight: "2.5em", borderRight: "1px solid lightgray"}}>
                                <img className={"red-folder-svg"} src={redFolder} />
                                <p style={{wordBreak: "break-all"}} className="poppins bold">{props.idMapping[props.folderId].name}</p>
                            </div>
                            <div style={{marginLeft: "1em", display: "flex", flexDirection: "column"}}>
                                <p style={{ textAlign: "left", wordBreak: "break-all", marginBottom: "2em"}} className="poppins bold"><span className="lato">Name : </span>{props.idMapping[props.folderId].name}</p>
                                <p style={{ textAlign: "left", wordWrap: "break-word"}} className="poppins bold"><span className="lato">Date Created : </span>{props.idMapping[props.folderId].date_created}</p>
                            </div>
                        </div>
